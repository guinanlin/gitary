...
import type { AIContext } from "../../interfaces/context-provider.interface";
...
export function createWorkspaceContextTool(context: IToolContext) {
  return tool({
    ...
    execute: async ({ includeBrowserTab = true, includeEditor = true, includeProject = false }: z.infer<typeof workspaceContextParams>): Promise<string> => {
      ...
      if (!context.contextProvider) {
        console.warn("[get_workspace_context] contextProvider not available");
      ...
      try {
        ...
        const contextPromise = Promise.resolve().then(() =>
          context.contextProvider!.getFullContext({
            includeBrowserTab,
            includeEditor,
            includeProject,
          })
        ...
        const aiContext = await Promise.race([contextPromise, timeoutPromise]);
        ...
        const result = formatWorkspaceContext(aiContext);
...
export const getWorkspaceContextTool = createWorkspaceContextTool;
