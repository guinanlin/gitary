...
class AgentConfiguration:
    ...
    def __post_init__(self) -> None:
        ...
        # Validate and set project_id (optional for local development)
        ...
        # Set staging bucket (optional for local development)
        self.staging_bucket = os.environ.get("GOOGLE_CLOUD_STAGING_BUCKET")

    @property
    def internal_agent_name(self) -> str:
        """
        Return the internal agent name.
        
        Fixed to 'main_task' for this agent.
        """
        return "main_task"
...
# Create main configuration (lazy initialization for local development)
config = AgentConfiguration()

# Only initialize Vertex AI if project_id is available (for deployment)
# This allows local development without Google Cloud configuration
