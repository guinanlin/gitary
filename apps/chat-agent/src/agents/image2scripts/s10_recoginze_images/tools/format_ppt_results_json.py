...
def format_ppt_results_json(
    total_images: int,
    image_results: List[Dict[str, Any]]
) -> Dict[str, Any]:
    """将PPT处理结果格式化为简化JSON格式（只包含图片地址和口播文案）
    
    这个工具用于确保输出的JSON格式完全正确且可解析。
    输出格式简化，只包含必要的字段：image_index, image_url, narration_script
    ...
    Example:
        >>> result = format_ppt_results_json(
        ...     total_images=2,
        ...     image_results=[
        ...         {
        ...             "image_index": 1,
    ...
    try:
        ...
        for idx, item in enumerate(image_results, 1):
            ...
            if not image_url.strip():
                errors.append(f"第{idx}项的image_url不能为空")
                continue
